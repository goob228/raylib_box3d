...
void Z_Free (void *ptr)
{
	...
	if (!ptr)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Free: NULL pointer");

	block = (memblock_t *) ( (byte *)ptr - sizeof(memblock_t));
	if (block->id != ZONEID)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Free: freed a pointer without ZONEID");
	if (block->tag == 0)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Free: freed a freed pointer");
...
static void *Z_TagMalloc (int size, int tag)
{
	...
	if (!tag)
		TraceLog(LOG_ERROR,  "Zone.c: Z_TagMalloc: tried to use a 0 tag");
...
static void Z_CheckHeap (void)
{
	...
	for (block = mainzone->blocklist.next ; ; block = block->next)
	{
		...
		if ( (byte *)block + block->size != (byte *)block->next)
			TraceLog(LOG_ERROR,  "Zone.c: Z_CheckHeap: block size does not touch the next block");
		if ( block->next->prev != block)
			TraceLog(LOG_ERROR,  "Zone.c: Z_CheckHeap: next block doesn't have proper back link");
		if (!block->tag && !block->next->tag)
			TraceLog(LOG_ERROR,  "Zone.c: Z_CheckHeap: two consecutive free blocks");
	}
}
...
void *Z_Malloc (int size)
{
	...
	if (!buf)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Malloc: failed on allocation of %i bytes",size);
	memset (buf, 0, size);
...
void *Z_Realloc(void *ptr, int size)
{
	...
	block = (memblock_t *) ((byte *) ptr - sizeof (memblock_t));
	if (block->id != ZONEID)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Realloc: realloced a pointer without ZONEID");
	if (block->tag == 0)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Realloc: realloced a freed pointer");
	...
	if (!ptr)
		TraceLog(LOG_ERROR,  "Zone.c: Z_Realloc: failed on allocation of %i bytes", size);
...
void Z_Print (memzone_t *zone)
{
	...
	TraceLog(LOG_NONE, "Zone.c: zone size: %i  location: %p\n",mainzone->size,mainzone);

	for (block = zone->blocklist.next ; ; block = block->next)
	{
		TraceLog(LOG_NONE, "Zone.c: block:%p    size:%7i    tag:%3i\n",
			block, block->size, block->tag);

		if (block->next == &zone->blocklist)
			break;			// all blocks have been hit
		if ( (byte *)block + block->size != (byte *)block->next)
			TraceLog(LOG_NONE, "Zone.c: ERROR: block size does not touch the next block\n");
		if ( block->next->prev != block)
			TraceLog(LOG_NONE, "Zone.c: ERROR: next block doesn't have proper back link\n");
		if (!block->tag && !block->next->tag)
			TraceLog(LOG_NONE, "Zone.c: ERROR: two consecutive free blocks\n");
	}
}
	...
	for (i = 0; i < hunk_numsegments && hunk_segments[i]->base < hunk_low_used; i++)
	{
		...
		for (ofs = 0; ofs < seg->used; )
		{
			...
			if (h->sentinel != HUNK_SENTINEL)
				TraceLog(LOG_ERROR,  "Zone.c: Hunk_Check: trashed sentinel");
			if (h->size < (int) sizeof(hunk_t) || h->size + ofs > seg->size)
				TraceLog(LOG_ERROR,  "Zone.c: Hunk_Check: bad size");
			ofs += h->size;
		}
	}
}
...
void Hunk_Print (bool all)
{
	...
	{
		TraceLog(LOG_NONE, "             Segments\n");
		TraceLog(LOG_NONE, "---------------------------------\n");
		TraceLog(LOG_NONE, "id :     offset :       size\n");
		TraceLog(LOG_NONE, "---------------------------------\n");
		for (i = 0; i < hunk_numsegments; i++)
			TraceLog(LOG_NONE, "%2i : %10i : %10i\n", i, hunk_segments[i]->base, hunk_segments[i]->size);
		TraceLog(LOG_NONE, "---------------------------------\n");
		TraceLog(LOG_NONE, "\n");
		TraceLog(LOG_NONE, "           Allocations\n");
		TraceLog(LOG_NONE, "---------------------------------\n");
	}

	if (all)
		TraceLog(LOG_NONE, "    offset :       size : name\n");
	else
		TraceLog(LOG_NONE, "allocs :       size : name\n");
	TraceLog(LOG_NONE, "---------------------------------\n");
	...
	if (all)
	{
		TraceLog(LOG_NONE, "%10s   %10i   USED (%d alloc%s)\n", "", hunk_low_used, PLURAL (totalblocks));
		TraceLog(LOG_NONE, "%10s   %10i   REMAINING\n", "", Hunk_Size () - hunk_low_used);
		TraceLog(LOG_NONE, "%10s   %10i   TOTAL\n", "", Hunk_Size ());
	}
	else
	{
		TraceLog(LOG_NONE, "%6i : %10i : USED\n", totalblocks, hunk_low_used);
		TraceLog(LOG_NONE, "%6s : %10i : REMAINING\n", "", Hunk_Size () - hunk_low_used);
		TraceLog(LOG_NONE, "%6s : %10i : TOTAL\n", "", Hunk_Size ());
	}
}
...
static int Hunk_SegForOfs (int ofs)
{
	...
	TraceLog(LOG_ERROR,  "Zone.c: Hunk_SegForOfs: bad offset %d (max: %d)", ofs, Hunk_Size ());
...
static int Hunk_SegForPtr (const void *ptr)
{
	...
	TraceLog(LOG_ERROR,  "Zone.c: Hunk_SegForPtr: bad pointer");
...
static void *Hunk_AllocInternal (int size, const char *name, hunkflags_t flags)
{
	...
	if (size < 0)
		TraceLog(LOG_ERROR,  "Zone.c: Hunk_Alloc: bad size: %i", size);
	...
	if (i == hunk_numsegments)
	{
		...
		if (hunk_numsegments == MAX_SEGMENTS)
			TraceLog(LOG_ERROR,  "Zone.c: Hunk_Alloc: segment overflow");
		...
		TraceLog(LOG_INFO, "Zone.c: Allocating new hunk segment: %.2lf MiB\n", newsize / 1048576.0);

		seg = (hunkseg_t *) malloc (sizeof (hunkseg_t) + newsize);
		if (!seg)
		{
			TraceLog(LOG_ERROR,  "Zone.c: Hunk_Alloc: failed on %i bytes", size);
			return NULL;
		}
...
void Hunk_FreeToLowMark (int mark)
{
	...
	if (mark < 0 || mark > hunk_low_used)
		TraceLog(LOG_ERROR,  "Zone.c: Hunk_FreeToLowMark: bad mark %i", mark);
...
// we are clearing up space at the bottom, so only allocate it late
	...
	if (new_cs)
	{
//		TraceLog(LOG_NONE, "Zone.c: cache_move ok\n");
	...
	{
//		TraceLog(LOG_NONE, "Zone.c: cache_move failed\n");
...
void Cache_UnlinkLRU (cache_system_t *cs)
{
	if (!cs->lru_next || !cs->lru_prev)
		TraceLog(LOG_ERROR,  "Zone.c: Cache_UnlinkLRU: NULL link");
...
{
	if (cs->lru_next || cs->lru_prev)
		TraceLog(LOG_ERROR,  "Zone.c: Cache_MakeLRU: active link");
...
// is the cache completely empty?
	if (!nobottom && cache_head.prev == &cache_head)
	{
		if ((ofs - LASTSEG->base) + size > LASTSEG->size)
			TraceLog(LOG_ERROR,  "Zone.c: Cache_TryAlloc: %i is greater then free hunk", size);
...
{
	TraceLog(LOG_INFO, "Zone.c: %4.1f megabyte data cache\n", (Hunk_Size () - hunk_low_used) / (float)(1024*1024) );
}

void CmdCache_Flush(Command* cmd)
{
    Cache_Flush();
}
...
void Cache_Init (void)
{
	...
	setCommandCallBack(&cmd_flush, CmdCache_Flush);
}
...
void Cache_Free (cache_user_t *c, bool freetextures) //johnfitz -- added second argument
{
	...
	if (!c->data)
		TraceLog(LOG_ERROR,  "Zone.c: Cache_Free: not allocated");
...
void *Cache_Alloc (cache_user_t *c, int size, const char *name)
{
	...
	if (c->data)
		TraceLog(LOG_ERROR,  "Zone.c: Cache_Alloc: already allocated");

	if (size <= 0)
		TraceLog(LOG_ERROR,  "Zone.c: Cache_Alloc: size %i", size);
...
// find memory for it
	while (1)
	{
		...
		if (cs)
		{
			strncpy (cs->name, name, CACHENAME_LEN);
			c->data = (void *)(cs+1);
			cs->user = c;
			break;
		}
	...
	// free the least recently used cahedat
		if (cache_head.lru_prev == &cache_head)
			TraceLog(LOG_ERROR,  "Zone.c: Cache_Alloc: out of memory"); // not enough memory at all
...
void Memory_Init (void *buf, int size)
{
	...
	if (p)
	{
		if (p < com_argc-1)
			...
			TraceLog(LOG_ERROR,  "Zone.c: Memory_Init: you must specify a size in KB after -zone");
	}
    */
    ...
    setCommandCallBack(&cmd_hunk_print, Hunk_Print_f);
}
